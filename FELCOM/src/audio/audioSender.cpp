...
void AudioSender::begin(uint8_t micPin, uint32_t sampleRateHz) {
    ...
    // Store sample rate for sine generator
    ...
    m_isStopped = false;  // Start in enabled state (will acquire from ADC by default)
...
bool AudioSender::update() {
    // Don't send anything if stopped
    if (m_isStopped) {
        return false;
    }
    
    if (m_useGeneratedAudio) {
        // Generate sine wave samples directly into a buffer
        static uint8_t generatedBuffer[ADCBuffer::BUFFER_SIZE];
        m_sineGenerator.generateSamples(generatedBuffer, ADCBuffer::BUFFER_SIZE);
        
        // Create and send audio packet
        AudioPacket packet;
        packAudioPacket(generatedBuffer, ADCBuffer::BUFFER_SIZE, &packet);
